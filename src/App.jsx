...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Apple SD Gothic Neo', 'Noto Sans KR', sans-serif", maxWidth: 480, margin: "0 auto" }}>
...
function Center({ children }) {
  return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", fontSize: 15, color: C.sub }}>{children}</div>;
}

function LoginScreen({ login }) {
  return (
    <div style={{ minHeight: "100vh", background: "#FFD966", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32 }}>
    ...
    <div style={{ padding: "24px 16px 40px" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {reviewItems.length === 0 ? (
          <div style={{ ...S.card, color: C.sub, fontSize: 14, padding: "14px 20px" }}>✅ 오늘 복습할 내용이 없어요!</div>
        ) : (
    ...
    <div style={{ padding: "24px 16px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {srcLessons.map(lesson => {
          ...
          const allDone = studied === lessonItems.length && lessonItems.length > 0;
          return (
...
function StudyScreen({ go, nav, lessons, items, progress, setProgress, setStudyDays }) {
  ...
  if (done) return (
    <div style={{ padding: 32, textAlign: "center" }}>
      ...
      <div style={{ fontWeight: 800, fontSize: 18, marginBottom: 8 }}>레슨 완료!</div>
      <div style={{ color: C.sub, fontSize: 14, marginBottom: 24 }}>{lessonItems.length}문장 학습했어요</div>
      <button onClick={() => go("lesson")} style={{ ...S.btn, background: C.primary, color: "#fff", width: "100%", padding: 14, fontSize: 15 }}>← 레슨 목록으로</button>
    </div>
  );
  ...
  if (phase === "preview") {
    ...
    return (
      <div style={{ padding: "24px 16px", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          ...
          <button onClick={() => go("lesson")} style={{ ...S.btn, background: "#FEE2E2", color: C.danger, padding: "5px 12px", fontSize: 12 }}>그만하기</button>
        </div>
        {ytId && previewIdx === 0 ? (
          ...
          <>
            <div style={{ fontSize: 12, color: C.sub, textAlign: "center", marginBottom: 12 }}>미리보기 {ytId ? previewIdx : previewIdx + 1} / {lessonItems.length}</div>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
              <div style={{ ...S.card, width: "100%", textAlign: "center", padding: 28 }}>
                <div style={{ fontSize: 18, fontWeight: 800, color: C.text, lineHeight: 1.6, marginBottom: 10 }}>{item.English}</div>
                <div style={{ color: C.sub, fontSize: 15 }}>{item.Korean}</div>
                <button onClick={() => speak(item.English)} style={{ ...S.btn, background: C.pill, color: C.primary, marginTop: 16, fontSize: 13 }}>🔊 듣기</button>
              </div>
    ...
    <div style={{ padding: "24px 16px", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <div style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
          ...
          <button onClick={() => { setStudyDays(prev => prev.includes(today()) ? prev : [...prev, today()]); go("lesson"); }} style={{ ...S.btn, background: "#FEE2E2", color: C.danger, padding: "5px 12px", fontSize: 12 }}>그만하기</button>
        </div>
      ...
      <div style={{ ...S.card, textAlign: "center", padding: 24, marginBottom: 16, minHeight: 140, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: C.sub, fontSize: 11, marginBottom: 8, fontWeight: 600 }}>다음을 영어로 말해보세요</div>
        <div style={{ fontSize: 18, fontWeight: 800, color: C.text, lineHeight: 1.5 }}>{item.Korean}</div>
        <button onClick={() => speak(item.English)} style={{ ...S.btn, background: C.pill, color: C.primary, marginTop: 12, fontSize: 13 }}>🔊 정답 듣기</button>
      </div>
      {!submitted ? (
        <>
        ...
        <>
          <div style={{ ...S.card, flex: 1, border: `2px solid ${correct ? C.success : C.danger}`, marginBottom: 16, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
            <div style={{ fontSize: 56, marginBottom: 28, filter: correct ? "invert(59%) sepia(52%) saturate(400%) hue-rotate(95deg) brightness(95%) contrast(90%)" : "none" }}>{correct ? "⭕" : "❌"}</div>
            <div style={{ fontWeight: 700, color: C.text, fontSize: 16, textAlign: "center", lineHeight: 1.5 }}>{item.English}</div>
            {answer && <div style={{ color: C.sub, fontSize: 12, marginTop: 8, textAlign: "center" }}>내 답: {answer}</div>}
          </div>
...
function ReviewScreen({ go, reviewItems, setProgress, setStudyDays }) {
  ...
  if (idx >= reviewItems.length) return (
    <div style={{ padding: 32, textAlign: "center" }}>
      <div style={{ fontSize: 48, marginBottom: 12 }}>🎉</div>
      <div style={{ fontWeight: 800, fontSize: 18, marginBottom: 16 }}>복습 완료!</div>
      <button onClick={() => go("home")} style={{ ...S.btn, background: C.primary, color: "#fff", width: "100%", padding: 14 }}>홈으로</button>
    </div>
  );
  ...
  return (
    <div style={{ padding: "24px 16px", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ ...S.card, marginBottom: 12, background: "#FEF9C3", border: `1px solid ${C.warn}`, padding: "12px 16px" }}>
      ...
      <div style={{ ...S.card, textAlign: "center", padding: 24, marginBottom: 16 }}>
        <div style={{ color: C.sub, fontSize: 11, marginBottom: 8, fontWeight: 600 }}>영어로 말해보세요</div>
        <div style={{ fontSize: 18, fontWeight: 800, color: C.text, lineHeight: 1.5 }}>{item.Korean}</div>
        <button onClick={() => speak(item.English)} style={{ ...S.btn, background: C.pill, color: C.primary, marginTop: 12, fontSize: 13 }}>🔊 정답 듣기</button>
      </div>
      {!submitted ? (
